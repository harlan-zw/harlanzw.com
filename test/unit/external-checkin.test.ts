...
import homeCheck from '../../checks/external/home'
...
const identity = { site: 'harlanzw.com', environment: 'production', deployment: 'release-1' }
const checks = [reportCheck, homeCheck, sentryCheck]
...
function siteReport(deployment = identity.deployment, observedAt = now) {
  return runChecks(['content.published'].map(id => defineCheck({ id, run: () => pass({}) })), { now: observedAt, identity: { ...identity, deployment } })
}
...
it('keeps absent credentials visible as incomplete coverage', async () => {
  ...
  expect(result.exitCode).not.toBe(0)
  expect(request.mock.calls.every(([url]) => url === 'https://harlanzw.com/')).toBe(true)
})
