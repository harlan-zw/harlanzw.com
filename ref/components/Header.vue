        ...
        <nav aria-label="Main Menu" class="opacity-75">
          <ul class="flex lg:(space-x-7 mr-15) sm:space-x-2 space-x-1 md:mr-7 items-center">
            <li v-for="(link, key) in nav" :key="key">
              <NuxtLink v-slot="data" :to="link.path" class="!inline-flex items-center group" :title="`Visit ${link.title} Page`">
                {{ data }}
                <div :class="data?.isActive ? ['sm:bg-green-50', 'text-green-800', 'dark:(sm:bg-green-800/10 text-green-50)'] : []" class="md:(px-3 py-2) px-1 py-1 rounded">
                  <svg
                    v-if="link.icon"
                    v-bind="link.icon"
                    class="hidden sm:inline-block text mr-2 mb-1 transition opacity-75 group-hover:opacity-100"
                    :class="data?.isActive ? 'opacity-100' : 'opacity-75'"
                  />
                  {{ link.title }}
                </div>
